...
自定义背景支持按视口方向选择资源：`背景方向模式` 默认是 `auto`，会在横竖屏切换时自动更新；也可以强制使用横屏或竖屏。亮色 / 暗色的横屏、竖屏地址留空时，会回退到对应的亮色 / 暗色背景地址，因此原有随机壁纸 API 无需改配置，静态壁纸则可分别填写四个方向地址。每个地址字段支持填写多张图片，用换行、英文逗号、中文顿号 `、`、竖线或分号分隔。
